...
echo "#####################################################################################"
echo "Alarm post"
ceilometer alarm-threshold-create --name alarm_test_plan -m image --period 10 --statistic avg --comparison-operator lt --threshold 0.9 1>/dev/null && ceilometer alarm-threshold-create --name alarm_test_plan_2 -m image --period 10 --statistic avg --comparison-operator lt --threshold 1.1 1>/dev/null
st=$?
if [ $st -eq 0 ]; then
    alarm_create="true"
fi
...
if [ $alarm_create == "true" ]; then
    ...
    echo "#####################################################################################"
    echo "Alarm delete"
    ceilometer alarm-delete $(ceilometer alarm-list | awk '/alarm_test_plan_2/{print $2}') 1>/dev/null && ceilometer alarm-delete $(ceilometer alarm-list | awk '/alarm_test_plan/{print $2}') 1>/dev/null
    check_return_code_after_command_execution "$?"
    echo "#####################################################################################"
else
    ${SETCOLOR_FAILURE}
